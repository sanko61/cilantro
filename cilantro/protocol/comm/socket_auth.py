...
class SocketAuth:
    ...
    def __init__(self, reset_auth_folder=False):
        self.keyname = Keys.public_key.hex()
        self.base_dir = 'certs/{}'.format(os.getenv('HOST_NAME', self.keyname))
        self.default_domain_dir = 'authorized_keys'
        self.authorized_keys_dir = join(self.base_dir, self.default_domain_dir)
        if reset_auth_folder:
            self.reset_folder()
        self.add_public_key(public_key=Keys.public_key)
        self.secure_context, self.auth = self.secure_context(async=True)
...
# handshake.py
# auth.py
    def add_public_key(self, public_key=None, vk=None, domain='*'):
        ...
        public_key_dir = join(self.base_dir, self.default_domain_dir if domain == '*' else domain)
...
# handshake.py
# auth.py
    def remove_public_key(self, public_key=None, vk=None, domain='*'):
        assert public_key or vk, 'No public key or vk provided'
        if vk: public_key = Keys.vk2pk(vk)
...
# /Users/lamden/lamden/cilantro/cilantro/protocol/comm/socket_manager.py
    def configure_auth(self, domain='*'):
        ...
        self.auth.configure_curve(domain=domain, location=location)

# /Users/lamden/lamden/cilantro/cilantro/protocol/comm/socket_manager.py
# this should not be creating context again - raghu
# should be part of zmq/socket utils - raghu ?
    def secure_context(self, context=None, async=False):
        if async:
            ctx = context or zmq.asyncio.Context()
            auth = AsyncioAuthenticator(ctx)
            auth.log = self.log # The constructor doesn't have "log" like its synchronous counter-part
        else:
...
# this should not be creating context again - raghu
# should be part of zmq/socket utils - raghu ?
    def secure_socket(self, sock, secret, public_key, domain='*'):
        sock.curve_secretkey = secret
        sock.curve_publickey = public_key
        if domain != '*':
            sock.zap_domain = domain.encode()
            public_key_dir = join(self.base_dir, domain)
        else:
            public_key_dir = join(self.base_dir, self.default_domain_dir)
        os.makedirs(public_key_dir, exist_ok=True)
        return sock
