...
class TestElection(SmartContractTestCase):
    ...
    def test_create(self, election):
        election_id = election.create_election(types.timedelta(seconds=1), int)
    ...
    def test_get_election(self, election):
        election_id = election.create_election(types.timedelta(seconds=37), int)
        e = election.get_election(election_id)
        self.assertEqual(e['opened_ts'], e['expire_on']-types.timedelta(seconds=37))

    @contract('election')
    def test_create_election_exhaustion(self, election):
        election_id = election.create_election(types.timedelta(seconds=1), int)
        with self.assertRaises(Exception) as context:
            election.create_election(types.timedelta(seconds=1), int)
    ...
    def test_pass(self, election):
        election_id = election.create_election(types.timedelta(seconds=10), int)
