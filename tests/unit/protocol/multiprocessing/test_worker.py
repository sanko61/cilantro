...
class WorkerTester:
    ...
    def test(func):
        ...
        # @mock.patch("cilantro.protocol.multiprocessing.worker.zmq", autospec=True)
        # @mock.patch("cilantro.protocol.multiprocessing.worker.zmq.asyncio", autospec=True)
...
class TestWorker(TestCase):
    ...
    def setUp(self, *args):
        Keys.setup('A' * 64)
        self.worker = Worker(name='TestWorker')

    @WorkerTester.test
    def test_open_envelope_valid(self, *args):
        Keys.setup('B' * 64)
        test_msg = TestMessage.create()
        env = Envelope.create_from_message(message=test_msg)
    ...
    def test_open_envelope_sender_valid(self, *args):
        Keys.setup(TESTNET_DELEGATES[0]['sk'])
        test_msg = TestMessage.create()
        env = Envelope.create_from_message(message=test_msg)
        env_binary = env.serialize()
